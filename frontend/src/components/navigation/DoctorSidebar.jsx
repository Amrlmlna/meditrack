...
import { FiX, FiHome, FiCalendar, FiUser, FiLogOut } from "react-icons/fi"

const DoctorSidebar = ({ open, onClose }) => {
  const { logout } = useAuth()

  const navItems = [
    { name: "Dashboard", path: "/doctor/dashboard", icon: <FiHome className="h-5 w-5" /> },
    { name: "Jadwal Konsultasi", path: "/doctor/consultations", icon: <FiCalendar className="h-5 w-5" /> },
    { name: "Profil", path: "/doctor/profile", icon: <FiUser className="h-5 w-5" /> },
  ]
