...
const getHospitalList = async ({ type, provinceid, cityid }) => {
  try {
    ...
    const endpoint = `rumah_sakit?jenis=${type}&propinsi=${provinceid}&kabkota=${
      cityid || ""
    }`;
  ...
  for (let i = 1; i <= count; i++) {
    ...
    if (type == 2) {
      ...
      hospitals.push({
        id,
        name: `RS ${
          ["Umum", "Swasta", "Daerah"][Math.floor(Math.random() * 3)]
        } ${
        ...
        address: `Jl. ${
          ["Sudirman", "Gatot Subroto", "Ahmad Yani", "Diponegoro"][
            Math.floor(Math.random() * 4)
          ]
      ...
      hospitals.push({
        id,
        name: `RS COVID ${
          ["Darurat", "Khusus", "Rujukan"][Math.floor(Math.random() * 3)]
        } ${provinceName} ${i}`,
        address: `Jl. ${
          ["Kesehatan", "Perjuangan", "Pahlawan", "Merdeka"][
            Math.floor(Math.random() * 4)
          ]
        } No. ${Math.floor(Math.random() * 100) + 1}, ${provinceName}`,
        phone: `08${Math.floor(Math.random() * 10)}${Math.floor(
